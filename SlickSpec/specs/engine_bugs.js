var specsSelectorEngineBugs = function(context){
	...
	describe('Bugs', function(){
		...
		it('should return the elements with passed class', function(){
			...
			var tmpNode1;
			tmpNode1 = context.document.createElement('span');
			tmpNode1.setAttribute('class','b');
			tmpNode1.setAttribute('className','b');
			testNode.appendChild(tmpNode1);

			var tmpNode2;
			tmpNode2 = context.document.createElement('span');
			tmpNode2.setAttribute('class','b');
			tmpNode2.setAttribute('className','b');
			testNode.appendChild(tmpNode2);
		...
		it('should return the element with passed id even if the context is not in the DOM', function(){
			...
			tmpNode = context.document.createElement('input');
			tmpNode.setAttribute('id', 'someuniqueid');
	...
	describe('Bugs Fixed', function(){
		...
		describe('SELECT [name]', function(){
			...
			it('Should match name attribute', function(){
				...
				tmpNode2 = context.document.createElement('input');
				tmpNode2.setAttribute('id',  'getelementsbyname');
				tmpNode2.setAttribute('type','password');
				testNode.appendChild(tmpNode2);

				tmpNode1 = context.document.createElement('input');
				tmpNode1.setAttribute('name','getelementsbyname');
				tmpNode1.setAttribute('type','text');testNode.appendChild(tmpNode1);
			...
			it('Should NOT match id attribute', function(){
				...
				tmpNode1 = context.document.createElement('input');
				tmpNode1.setAttribute('name','getelementsbyname');
				tmpNode1.setAttribute('type','text');
				testNode.appendChild(tmpNode1);
		...
		describe('SELECT #', function(){
			...
			it('Should NOT match name attribute', function(){
				...
				tmpNode1 = context.document.createElement('input');
				tmpNode1.setAttribute('name','getelementbyid');
				tmpNode1.setAttribute('type','text');
				testNode.appendChild(tmpNode1);

				tmpNode2 = context.document.createElement('input');
				tmpNode2.setAttribute('id',  'getelementbyid');
				tmpNode2.setAttribute('type','password');
				testNode.appendChild(tmpNode2);
			...
			it('Should match id attribute, even when another element has that [name]', function(){
				...
				tmpNode3 = context.document.createElement('input');
				tmpNode3.setAttribute('name','getelementbyid');
				tmpNode3.setAttribute('type','text');
				testNode.appendChild(tmpNode3);
		...
		describe('MATCH :nth-child', function() {

			beforeEach(setup);
			afterEach(teardown);

			if( !isXML )
			it('Should match nodes with the same selector that they were selected', function(){
				teardown();setup();
